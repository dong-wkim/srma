# Custom R functions for Meta-Analysis

①  Custom function to **install packages *as needed* and load libraries**:
...
```R
library(metafor)
setwd("G:/My Drive/4. Research (PhD)/S-QT B-QT SR MA/3. Data collection")
```

---
③ Custom function for **generating forest plots** using continuous data

Note: make sure to put the R file in the parent directory with child directories data/xlsx/continuous and data/xlsx/dichotomous.
...
④ Custom function for **generating forest plots** using dichotomous data
...
# Name your CSV/Excel files with continuous and dichotomous data with underscores instead of spaces.
# Then input the names of the CSV files without the .csv/.xlsx extension (coconut_sales.csv becomes "coconut_sales")
